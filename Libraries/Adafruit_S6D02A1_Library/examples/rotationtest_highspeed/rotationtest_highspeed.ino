...
void rotateFillcircle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillCircle(10, 30, 10, S6D02A1_YELLOW);
...
void rotateDrawcircle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawCircle(10, 30, 10, S6D02A1_YELLOW);
...
void rotateFillrect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillRect(10, 20, 10, 20, S6D02A1_GREEN);
...
void rotateDrawrect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawRect(10, 20, 10, 20, S6D02A1_GREEN);
...
void rotateFastline(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawFastHLine(0, 20, tft.width(), S6D02A1_RED);
    tft.drawFastVLine(20, 0, tft.height(), S6D02A1_BLUE);
...
void rotateLine(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawLine(tft.width()/2, tft.height()/2, 0, 0, S6D02A1_RED);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotatePixel(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawPixel(10,20, S6D02A1_WHITE);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateTriangle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawTriangle(20, 10, 10, 30, 30, 30, S6D02A1_GREEN);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateFillTriangle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillTriangle(20, 10, 10, 30, 30, 30, S6D02A1_RED);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateRoundRect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawRoundRect(20, 10, 25, 15, 5, S6D02A1_BLUE);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateFillRoundRect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillRoundRect(20, 10, 25, 15, 5, S6D02A1_CYAN);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateChar(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawChar(25, 15, 'A', S6D02A1_WHITE, S6D02A1_WHITE, 1);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
